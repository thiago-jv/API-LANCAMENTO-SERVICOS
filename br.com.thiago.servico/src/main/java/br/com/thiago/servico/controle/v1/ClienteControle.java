...
public class ClienteControle implements Serializable {
    ...
    @PostMapping
    public ResponseEntity<ClienteResponseDTO> criar(@ApiParam(name = "corpo", value = "Representação de um novo 'Cliente'") @Valid @RequestBody ClientePostDTO cliente, HttpServletResponse response) {
        return ResponseEntity.status(HttpStatus.CREATED).body(clienteMapper.toClienteResponseDTO(clienteService.salvar(clienteMapper.toCliente(cliente))));
    }
